...
let CurrentMinuteElements = (function(){
  ...
  function syncGlobal( minutes ){
    /*
		...
		*/
		const currentMinuteSelectors = document.querySelectorAll( '.amplitude-current-minutes' );

		/*
			Set the current minute selector's inner html to minutes passed in.
		*/
		for( let i = 0; i < currentMinuteSelectors.length; i++ ){
      let playlist = currentMinuteSelectors[i].getAttribute('data-amplitude-playlist');
      let songIndex = currentMinuteSelectors[i].getAttribute('data-amplitude-song-index');
  ...
  function syncPlaylist( minutes ){
    /*
			Get all of the minute selectors
		*/
		const currentMinutePlaylistSelectors = document.querySelectorAll( '.amplitude-current-minutes[data-amplitude-playlist="'+config.active_playlist+'"]' );

		/*
			Set the current minute selector's inner html to minutes passed in.
		*/
		for( let i = 0; i < currentMinutePlaylistSelectors.length; i++ ){
      let songIndex = currentMinutePlaylistSelectors[i].getAttribute('data-amplitude-song-index');
  ...
  function syncSong( minutes ){
    if( config.active_playlist == null ){
      /*
  			Get all of the minute selectors
  		*/
  		const currentMinuteSongSelectors = document.querySelectorAll( '.amplitude-current-minutes[data-amplitude-song-index="'+config.active_index+'"]' );

  		/*
  			Set the current minute selector's inner html to minutes passed in.
  		*/
  		for( let i = 0; i < currentMinuteSongSelectors.length; i++ ){
        let playlist = currentMinuteSongSelectors[i].getAttribute('data-amplitude-playlist');
  ...
  function syncSongInPlaylist( minutes ){
    ...
    /*
			Get all of the minute selectors
		*/
		const currentMinutePlaylistSongSelectors = document.querySelectorAll( '.amplitude-current-minutes[data-amplitude-playlist="'+config.active_playlist+'"][data-amplitude-song-index="'+activePlaylistIndex+'"]' );
