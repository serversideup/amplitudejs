...
let CurrentSecondElements = (function(){
  ...
  function syncGlobal( seconds ){
    /*
		...
		*/
		const currentSecondSelectors = document.querySelectorAll( '.amplitude-current-seconds' );

		/*
			Set the current second selector's inner html to seconds passed in.
		*/
		for( let i = 0; i < currentSecondSelectors.length; i++ ){
      let playlist = currentSecondSelectors[i].getAttribute('data-amplitude-playlist');
      let songIndex = currentSecondSelectors[i].getAttribute('data-amplitude-song-index');
  ...
  function syncPlaylist( seconds ){
    /*
			Get all of the second selectors
		*/
		const currentSecondPlaylistSelectors = document.querySelectorAll( '.amplitude-current-seconds[data-amplitude-playlist="'+config.active_playlist+'"]' );

		/*
			Set the current second selector's inner html to seconds passed in.
		*/
		for( let i = 0; i < currentSecondPlaylistSelectors.length; i++ ){
      let songIndex = currentSecondPlaylistSelectors[i].getAttribute('data-amplitude-song-index');
  ...
  function syncSong( seconds ){
    if( config.active_playlist == null ){
      /*
  			Get all of the second selectors
  		*/
  		const currentSecondSongSelectors = document.querySelectorAll( '.amplitude-current-seconds[data-amplitude-song-index="'+config.active_index+'"]' );

  		/*
  			Set the current second selector's inner html to seconds passed in.
  		*/
  		for( let i = 0; i < currentSecondSongSelectors.length; i++ ){
        let playlist = currentSecondSongSelectors[i].getAttribute('data-amplitude-playlist');
  ...
  function syncSongInPlaylist( seconds ){
    ...
    /*
			Get all of the second selectors
		*/
		const currentSecondPlaylistSongSelectors = document.querySelectorAll( '.amplitude-current-seconds[data-amplitude-playlist="'+config.active_playlist+'"][data-amplitude-song-index="'+activePlaylistIndex+'"]' );
