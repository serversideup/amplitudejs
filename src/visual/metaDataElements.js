...
let MetaDataElements = (function(){
  /**
	...
	function displayMetaData(){
		...
		*/
		let songInfoElements = document.querySelectorAll('[data-amplitude-song-info]');
		...
		*/
		for( let i = 0; i < songInfoElements.length; i++ ){
			...
			*/
			let playlist = songInfoElements[i].getAttribute('data-amplitude-playlist');
			let songIndex = songInfoElements[i].getAttribute('data-amplitude-song-index');
			...
			*/
			if( songIndex == null && ( config.active_playlist == playlist || ( playlist == null && songIndex == null ) ) ){
				/*
	...
	function displayPlaylistMetaData(){
		...
		*/
		let playlistInfoElements = document.querySelectorAll('[data-amplitude-playlist-info]');
		...
		*/
		for( let i = 0; i < playlistInfoElements.length; i++ ){
			/*
				...
				key.
			*/
			let info = playlistInfoElements[i].getAttribute('data-amplitude-playlist-info');
			let playlist = playlistInfoElements[i].getAttribute('data-amplitude-playlist');

			if( config.playlists[playlist][info] != undefined ){
				if( imageMetaDataKeys.indexOf( info ) >= 0 ){
					playlistInfoElements[i].setAttribute('src', config.playlists[playlist][info]);
				}else{
				...
				*/
				if( imageMetaDataKeys.indexOf( info ) >= 0 ){
					if( config.default_playlist_art != '' ){
						playlistInfoElements[i].setAttribute('src', config.default_playlist_art);
					}else{
  ...
  function setFirstSongInPlaylist( song, playlist ){
    ...
    */
    ...
    /*
      Get all of the song info elements
    */
    let songInfoElements = document.querySelectorAll('[data-amplitude-song-info][data-amplitude-playlist="'+playlist+'"]');
    ...
    */
    for( let i = 0; i < songInfoElements.length; i++ ){
      ...
      */
      let elementPlaylist = songInfoElements[i].getAttribute('data-amplitude-playlist');
	 ...
	 for( let i = 0; i < songInfoElements.length; i++ ){
		 ...
		 if( songIndex != null && playlist == null ){
			 ...
			 */
			 if( config.songs[songIndex][info] != undefined ){
				 ...
				 */
				 if( imageMetaDataKeys.indexOf( info ) >= 0 ){
					 songInfoElements[i].setAttribute('src', config.songs[songIndex][info]);
				 }else{
		 ...
		 if( songIndex != null && playlist != null ){
			 ...
			 if( config.playlists[playlist].songs[songIndex][info] != undefined ){
				 if( imageMetaDataKeys.indexOf( info ) >= 0 ){
					 songInfoElements[i].setAttribute( 'src', config.playlists[playlist].songs[songIndex][info] );
				 }else{
					 songInfoElements[i].innerHTML = config.playlists[playlist].songs[songIndex][info];
				 }
			 }
		 }
	 }
