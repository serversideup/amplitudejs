...
let Visualizations = (function(){
  ...
  function run(){
    ...
    */
    let visualizationElements = document.querySelectorAll( '.amplitude-visualization' );
    ...
    */
    if( config.web_audio_api_available ){
      ...
      */
      if( Object.keys( config.visualizations.available ).length > 0
        && visualizationElements.length > 0 ){
          ...
          */
          for( let i = 0; i < visualizationElements.length; i++ ){
            ...
            */
            let playlist = visualizationElements[i].getAttribute('data-amplitude-playlist');
            let song = visualizationElements[i].getAttribute('data-amplitude-song-index');
  ...
  function runGlobalVisualization( element ){
    ...
    */
    ...
    let activeSongVisualizationIndex = config.active_index != null
                                     ? config.songs[ config.active_index ].visualization
                                     : config.playlists[ config.active_playlist ].songs[ config.playlists[ config.active_playlist ].active_index ].visualization ;

    /*
      If the active song visualization is defined and the visualization exists,
      use that visualization.
    */
    if( activeSongVisualizationIndex != undefined
    ...
    */
    }else if( globalVisualizationIndex != undefined
    ...
    */
      ...
      */
      let firstVisualization = Object.keys( config.visualizations.available ).length > 0 ? Object.keys( config.visualizations.available )[0] : null;
  ...
  function runPlaylistVisualization( element, playlist ){
    ...
    */
    if( playlist == config.active_playlist ){
      ...
      */
      let activeSongVisualizationIndex = config.playlists[ config.active_playlist ].songs[ config.playlists[ config.active_playlist ].active_index ].visualization;
      let activePlaylistVisualizationIndex = config.playlists[ config.active_playlist ].visualization;
      let globalVisualizationIndex = config.visualization;
      ...
      */
      }else if( activePlaylistVisualizationIndex != undefined
      && config.visualizations.available[ activePlaylistVisualizationIndex ] != undefined ){
  ...
  function runSongVisualization( element, song ){
    ...
    */
    if( song == config.active_index ){
      ...
      */
      let activeSongVisualizationIndex = config.songs[ config.active_index ].visualization;
      let globalVisualizationIndex = config.visualization;
  ...
  function addToActiveVisualizations( key, element ){
    ...
    visualization.setPreferences( config.visualizations.available[ key ]['preferences'] );
  ...
  function register( visualization, preferences ){
    ...
    */
    ...
    config.visualizations.available[ newVisualization.getID() ] = new Array();
	  config.visualizations.available[ newVisualization.getID() ]['object'] = visualization;
    config.visualizations.available[ newVisualization.getID() ]['preferences'] = preferences;
  }
    ...
    if( visualizationElements.length > 0 ){
      for( let x = 0; x < visualizationElements.length; x++ ){
        /*
          ...
          determine which one we run.
        */
        let playlist = visualizationElements[x].getAttribute('data-amplitude-playlist');
        let song = visualizationElements[x].getAttribute('data-amplitude-song-index');
  ...
  function displayGlobalBackup( element ){
    element.style.backgroundImage = "url("+config.active_metadata.cover_art_url+")";
  }
  ...
  function displaySongInPlaylistBackup( element, playlist, song ){
    if( config.active_playlist == playlist && config.playlists[active_playlist].active_index == song ){
      element.style.backgroundImage = "url("+config.active_metadata.cover_art_url+")";
    }
  }
