...
test( 'AmplitudeJS adds songs correctly to playlist songs array', () => {
  ...
  expect( config.playlists['emancipator'].songs[index].name ).toBe('Test New Song');
...
test( 'AmplitudeJS goes to the previous song when shuffled', () => {
  ...
  expect( config.active_metadata ).toBe( config.shuffle_list[ config.songs.length - 1 ] );
});
 ...
 */
 test( 'AmplitudeJS goes to the previous song in the playlist when shuffled', () => {
   ...
   expect( config.active_metadata ).toBe( config.playlists['emancipator'].shuffle_list[ config.playlists['emancipator'].songs.length - 1 ] );
 });
