...
test('AmplitudeJS Song Slider Elements adjust to current time', () => {
  ...
  SongSliderElements.sync(config.audio.currentTime, config.active_playlist, config.active_playlist != '' && config.active_playlist != null ? config.playlists[ config.active_playlist ].active_index : config.active_index );
  ...
  expect( document.getElementById('song-in-playlist-song-slider').value ).toBe( "0" );
...
test('AmplitudeJS Song Slider Elements adjust current Time In Playlist', () => {
  ...
  expect( document.getElementById('song-in-playlist-song-slider').value ).toBe( "45" );
  expect( document.getElementById('individual-song-slider').value ).toBe( "0" );
});
