...
test('AmplitudeJS Shuffles Songs Accordingly', () => {
  ...
  expect( document.getElementById( 'global-shuffle' ).classList.contains('amplitude-shuffle-on') );
  expect( config.shuffle_list.length ).toBe( config.songs.length );

  document.getElementById('global-shuffle').click();

  expect( document.getElementById( 'global-shuffle' ).classList.contains('amplitude-shuffle-off') );
...
test('AmplitudeJS Shuffles Playlist Songs Accordingly', () => {
  ...
  expect( document.getElementById( 'playlist-shuffle' ).classList.contains('amplitude-shuffle-on') );
  expect( config.playlists['trip_hop'].shuffle_list.length ).toBe( config.playlists['trip_hop'].songs.length );

  document.getElementById('playlist-shuffle').click();

  expect( document.getElementById( 'playlist-shuffle' ).classList.contains('amplitude-shuffle-off') );
