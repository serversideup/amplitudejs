...
test('AmplitudeJS Song Containers Get Proper Classes', () => {
  ...
  expect( document.getElementById('song-container').classList.contains('amplitude-active-song-container') ).toBe(true);

  Amplitude.playSongAtIndex(2);

  expect( document.getElementById('song-container').classList.contains('amplitude-active-song-container') ).toBe(false);
});

test('AmplitudeJS Song In Playlist Containers Get Proper Classes', () => {
  Amplitude.playPlaylistSongAtIndex( 2, 'trip_hop' );

  expect( document.getElementById('song-in-playlist-container').classList.contains('amplitude-active-song-container') ).toBe(true);

  Amplitude.playSongAtIndex(2);

  expect( document.getElementById('song-in-playlist-container').classList.contains('amplitude-active-song-container') ).toBe(false);
});
