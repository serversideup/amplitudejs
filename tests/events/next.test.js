...
test('AmplitudeJS Next Goes To Next Playlist Song', () => {
  ...
  expect( config.active_metadata.name ).toBe( config.playlists['emancipator'].songs[1].name );
});
...
test('AmplitudeJS Playlist Next Goes To Next Playlist Song', () => {
  ...
  expect( config.active_metadata.name ).toBe( config.playlists['trip_hop'].songs[1].name );
});
...
test('AmplitudeJS Playlist Next Goes To Next Playlist Shuffle Song', () => {
  ...
  expect( config.playlists['trip_hop'].active_index ).toBe( 1 );
  expect( config.active_metadata.name ).toBe( config.playlists['trip_hop'].shuffle_list[1].name );
});
